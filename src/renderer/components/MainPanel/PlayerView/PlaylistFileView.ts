...
class PlaylistFileView extends HTMLElement {
  ...
  private buildNoFiles() {
    ...
    noFiles.textContent = "This playlist is currently empty.";
    applyStyles(noFiles, {
      ...universalStyles,
      display: "flex",
  ...
  private onItemCloseButtonClicked(event: CustomEvent) {
    ...
    const customEvent = new CustomEvent("file-removed-from-playlist", {
      bubbles: true,
      detail: {
        index,
      },
...
customElements.define("playlist-file-view", PlaylistFileView);

export default PlaylistFileView;